...
namespace DSRawool.Controllers
{
    ...
    public class AccountsController : ControllerBase
    {
        ...
        public async Task<IActionResult> RegisterUser([FromBody] UserForRegistrationDto userForRegistration)
        {
            ...
            {
                {"token", StringHelper.Base64ForUrlEncode(token) },
                {"email", user.Email }
            };

                var callback = QueryHelpers.AddQueryString(userForRegistration.ClientURI, param);

                var message = new Message(new string[] { user.Email }, "Email Confirmation Link", callback, null);
        ...
        public async Task<IActionResult> EmailConfirmation([FromQuery] string email, [FromQuery] string token)
        {
            ...
            //var tokenHtmlDecoded = HttpUtility.UrlDecode(token);
            var confirmResult = await _userManager.ConfirmEmailAsync(user, StringHelper.Base64ForUrlDecode(token));
